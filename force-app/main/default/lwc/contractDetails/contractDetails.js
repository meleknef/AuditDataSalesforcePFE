...
//import getAuditContract from '@salesforce/apex/ListAudit.getAuditContract';
import sendReport from '@salesforce/apex/ListAuditPdf.sendReport';
import getCntra from '@salesforce/apex/ListAudit.getCntra';
import {ShowToastEvent} from 'lightning/platformShowToastEvent';
import sendmail from '@salesforce/apex/ScheduledMail.sendmail';
import { NavigationMixin } from 'lightning/navigation';
import getBackupCntrt from '@salesforce/apex/BackupFunctions.getBackupCntrt';
import backupcntr from '@salesforce/apex/BackupFunctions.backupcntr';


const columns = [
    { label: 'Name ', fieldName: 'Name', sortable: true },
    { label: 'Number ', fieldName: 'ObjectName__c', sortable: true }, 
...
export default class ContractDetails extends NavigationMixin (LightningElement) {
    ...
    @track wiredContBackup;
    @track wiredContracts;

    // eslint-disable-next-line @lwc/lwc/no-async-await
    /*async connectedCallback() {
        this.data = await getAuditContract({ amountOfRecords: 100 });
    }*/
/*********** Audit tab ********* */
    ...
    @wire(getCntra, {searchKey: '$searchKey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    bbbackup(event) {
        ...
        backupcntr({ sObj: row }).then(resultt => {
    ...
    @wire(getBackupCntrt, {searchKey: '$searchKey', sortByb: '$sortedByb', sortDirectionb: '$sortedDirectionb'})
    getBackupCntrt(resultt) {

        this.wiredContBackup = resultt;
        ...
        if (resultt.data) 
        {
          ...
          this.allData = this.itemsb.slice(0,this.pageSizeb); 
           ...
           this.backs = backs;
           console.log('dataaa : ' , this.wiredContBackup);
   ...
   */
    stopLoading(timeoutValue) {
        setTimeout(() => {
            ...
            refreshApex(this.wiredContBackup);
            this.loading = false;
        }, timeoutValue);
      }
