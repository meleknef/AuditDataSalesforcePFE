...
//import getAuditProduct from '@salesforce/apex/ListAudit.getAuditProduct';
import sendReport from '@salesforce/apex/ListAuditPdf.sendReport';
import getProd from '@salesforce/apex/ListAudit.getProd';
import {ShowToastEvent} from 'lightning/platformShowToastEvent';
import { NavigationMixin } from 'lightning/navigation';
import sendmail from '@salesforce/apex/ScheduledMail.sendmail';
import getBackupProd from '@salesforce/apex/BackupFunctions.getBackupProd';
import backupProd from '@salesforce/apex/BackupFunctions.backupProd';
...
export default class ProductDetails extends NavigationMixin (LightningElement){
    ...
    @track wiredProducts;
    @track wiredProdBackup;


    // eslint-disable-next-line @lwc/lwc/no-async-await
    async connectedCallback() {
        this.data = await getAuditProduct({ amountOfRecords: 100 });
    }
    ...
    @wire(getProd, {searchKey: '$searchKey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    bbbackup(event) {
        ...
        backupProd({ sObj: row }).then(resultt => {
    ...
    @wire(getBackupProd, {searchKey: '$searchKey', sortByb: '$sortedByb', sortDirectionb: '$sortedDirectionb'})
    getBackupProd(resultt) {

        this.wiredProdBackup = resultt;
        ...
        if (resultt.data) 
        {
          ...
          this.allData = this.itemsb.slice(0,this.pageSizeb); 
           ...
           this.backs = backs;
           console.log('dataaa : ' , this.wiredProdBackup);
   ...
   */
    stopLoading(timeoutValue) {
        setTimeout(() => {
            ...
            refreshApex(this.wiredProdBackup);
            this.loading = false;
        }, timeoutValue);
      }
